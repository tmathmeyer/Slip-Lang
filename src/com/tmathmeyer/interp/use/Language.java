...
public class Language
{
	...
	{
		ImmutableList<Macro> macros = input.map(a -> a.asExpression())
										   .filter(in -> in instanceof Macro)
										   .map(in -> (Macro)in);
		...
		ImmutableList<Expression> nonmacros = input.map(a -> a.asExpression())
												   .filter(a -> !(a instanceof Macro));
		
		ImmutableList<Expression> los = nonmacros.map(a -> a.desugar());
		
		ImmutableList<DefSansSet> defs = los.filter(in -> in instanceof DefSansSet)
											  .map(in -> (DefSansSet)in);
