	...
	{	
		switch(tokens.first().piece.charAt(0))
		{
			case '(':
				...
				if (tokens.isEmpty())
				{
					throw new RuntimeException("expected a ) at col:"+last.col_nom+", row:"+last.line_num+" but found nothing");
				}
				return new Pair<AST, ImmutableList<Token>>(tree, tokens.rest());
			case ')':
				System.out.println("error, encountered an unexpected \")\" at col:"+tokens.first().col_nom+", row:"+tokens.first().line_num);
				break;
			default:
